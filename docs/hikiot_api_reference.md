...
登录方式补充（外部人员说明，待正式接口契约确认）：海康侧默认使用账号密码登录，新设备首次登录需要手机号短信验证码，完成一次设备验证后可继续使用账号密码。当前仓库未实现或确认账号密码、短信发送、验证码校验接口，现有客户端仍通过登录网页获取 `www_token`。
...
- 当前代码的 `getMonthlyAttendance()` 没有使用这个接口，而是通过 `MonthlyAttendanceAggregator` 并发请求整月每日 V2 再聚合，默认最多 6 个请求同时执行。单日失败会记录到 `failedDates`，不会直接让整月聚合失败；月度页目前没有把失败日期作为明确的用户状态展示。
...
1. 启动刷新顺序已经由 `StartupRefreshCoordinator` 串行保证，但三个页面仍通过 `MainScreen` 的 `GlobalKey` 直接调用彼此的 State 方法，后续页面重构和状态测试成本较高。
2. `StorageService.clearAuthInfo()` 当前只清理 token 和用户名，没有清理 `teamNo/personNo/teamName/selectedTeam`；更换账号时，旧团队上下文可能在新会话初始化前短暂残留，需在登录切换场景补验证。
3. 每日页、月度页和后台提醒目前都复用每日 V2，但月度聚合的单日失败会进入 `failedDates`，页面没有明确展示部分数据缺失。
4. 每日 V2 的休息日和当天未完整打卡样本会缺字段，当前 Parser 已做 null-safe，页面和提醒仍需持续覆盖这些边界。
5. `ApiConstants` 定义了连接/读取超时秒数，但 `HikiotApiClient` 使用的 `http.Client` 没有实际应用超时配置，网络异常时的等待时长需要真机验证。
6. `NotificationService.rescheduleAllAlarms()` 已定义但在当前 Dart 代码中未发现主动调用点；开机恢复是否完全由 AlarmManager 插件接管，需要 Android 真机验证。
7. 当前仓库仍未实现海康账号密码、短信发送/校验和设备信任接口；登录方式优化必须先取得正式接口契约。
