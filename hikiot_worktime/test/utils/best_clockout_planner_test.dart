...
void main() {
  ...
  group('自由模式', () {
    test('下一个最佳时刻是最近的整十分位（6 分钟边界）', () {
      ...
      // 恰好在整点
      expect(
        BestClockOutPlanner.nextBestFreeTime(8 * 60, 14 * 60),
        14 * 60,
      );
    });
  });
  ...
  group('地铁模式', () {
    test('候选班次下班时刻 = 到站 - 6 分钟，取性价比最高', () {
      // 8:00 打卡，17:00 现在。方向① 17:02 班次 → 下班 16:56（已过，跳过）；
      // 17:06 班次 → 下班 17:00（现在）残差0最佳
    ...
    test('错过 17:00 后推荐下一个高性价比班次', () {
      // 17:01 现在：17:06 班次 → 下班 17:00 已过；17:11 班次 → 下班 17:05，
      // 工时 9h05m = 545 分钟，残差 = (5.45×100=544.99→544)/100-5.4 → 0.04 一般
      final plan = BestClockOutPlanner.bestMetroPlan(
        line: HanyuJinguMetro.lineWest,
        checkInMinutes: 8 * 60,
        nowMinutes: 17 * 60 + 1,
    ...
    test('当前能否赶上最近班次', () {
      ...
      expect(catchPlan!.trainTime, 17 * 60 + 6);
    });
